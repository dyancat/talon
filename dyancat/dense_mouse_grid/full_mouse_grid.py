...
class MouseSnapMillion:
    ...
    def setup(self, *, rect: Rect = None, screen_num: int = None):
        ...
        if self.mcanvas is not None:
            self.mcanvas.close()
        if self.mmouse_canvas is not None:
            self.mmouse_canvas.close()
        self.mmouse_canvas = canvas.Canvas.from_screen(screen)
        self.mcanvas = canvas.Canvas.from_screen(screen)
        if self.active:
            self.mcanvas.register("draw", self.draw)
            self.mmouse_canvas.register("draw", self.draw_mouse)
            self.mcanvas.freeze()
    ...
    def show(self):
        ...
        ctrl.mouse_move(self.rect.x + superblock_size / 2, self.rect.y + superblock_size / 2)
    ...
    def close(self):
        ...
        self.mmouse_canvas.unregister("draw", self.draw_mouse)
        #self.mcanvas.close()
        #self.mcanvas = None
        self.img = None
        self.input_so_far = ""
        ...
        def draw_superblock():
            ...
            for row in range(0, int(self.rect.height) // superblock_size + 1):
                for col in range(0, int(self.rect.width) // superblock_size + 1):
                    ...
                    # commenting out drawing overlay for whole screen
    ...
    def draw_mouse(self, canvas):
        ...
        paint.color = (0, 0, 0, 0x66)
        canvas.draw_circle(x, y, 150)

        paint.color = (0, 0, 0, 0)
        paint.blendmode = paint.Blend.SRC
        canvas.draw_circle(x, y, 100)
